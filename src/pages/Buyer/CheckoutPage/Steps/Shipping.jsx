...
const formSchema = Joi.object({
  buyerAddress: Joi.string().required().trim().strict().messages({
...
function Shipping({
  ...
  const handleUpdateShipping = (data) => {
    setCheckoutInfo({
      ...checkoutInfo,
      shipping: shippingData.map(i => i.find(s => s.type === data.shippingMethod))
    })
    setStep(3)
  }
    ...
    <div className="">
      ...
      {clusterOrders.map((clusterOrder, index) => (
        <div key={index} className="grid grid-cols-12 gap-12">
          <div className="my-10 border-[2px] border-b-[#ddd] rounded-md p-4 col-span-9">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleUpdateShipping)} className="space-y-8">
                <div className='grid grid-cols-2 gap-10'>
                  ...
                  <div>
                    <div className="text-mainColor1-600 font-medium text-lg mb-2">Địa chỉ gửi hàng</div>
                    <FormField
                      control={form.control}
                      name="buyerAddress"
                      render={({ field }) => (
                        <FormItem>
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <SelectTrigger className="w-full">
                              <SelectValue placeholder="Chọn địa chỉ gửi hàng" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectGroup>
                                <SelectItem value="0">{shopAddresses[index]}</SelectItem>
                              </SelectGroup>
                            </SelectContent>
                          </Select>
                          <FormDescription>
