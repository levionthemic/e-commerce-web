            ...
            <div className='col-span-9'>
              <div className='flex items-center justify-between mx-20 mb-2 mt-4'>
                ...
                <div className='font-semibold text-mainColor1-600 flex items-center gap-4'>
                  <img src={checkoutInfo?.shipping[index].type === 'ghn' ? ghnLogo : ghtkLogo} alt="" width={20} height={20}/>
                  {checkoutInfo?.shipping[index].type === 'ghn' && 'Giao hàng nhanh'}
                  {checkoutInfo?.shipping[index].type === 'ghtk' && 'Giao hàng tiết kiệm'}
                </div>
              </div>
                  ...
                  <div key={index} className='flex items-center gap-2 my-4 overflow-hidden'>
                    ...
                    <div className='flex items-center justify-between flex-1'>
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <span className='text-lg line-clamp-1 text-mainColor2-800 leading-none'>{product?.productName}</span>
                            </TooltipTrigger>
                            <TooltipContent>
                      ...
                      <div className='flex flex-col lg:flex-row lg:items-center lg:gap-4'>
                        ...
                        <span className='text-muted-foreground'>x {product?.price.toLocaleString('vi-VN')}<sup>đ</sup></span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className='border border-b-[#ddd] rounded-md p-4 shadow-md'>
                ...
                <div className='px-2'>
                  ...
                  <div className='flex items-center justify-between text-sm my-2'>
                    ...
                    <span className='font-bold text-red-600'>
                      {(checkoutInfo?.shipping?.[index]?.detail?.total || 0)?.toLocaleString('vi-VN')}
                      <sup>đ</sup>
                    </span>
                  </div>
                  <div className='flex items-center justify-between text-sm my-2'>
                ...
                <div>
                  ...
                  <div className='text-red-600 text-right text-xl font-bold'>
                    {(totalPrice(clusterOrder) + (checkoutInfo?.shipping?.[index]?.detail?.total || 0)).toLocaleString('vi-VN')}
                    <sup>đ</sup>
                  </div>
                </div>
              </div>
