...
function SiderAdmin() {
  ...
  useEffect(() => {
    axiosApi.get('/api/v1' + systemConfig.prefixAdmin + '/user/get-info').then((data) => {
  ...
  useEffect(() => {
    ...
    switch (pathname) {
    case `${systemConfig.prefixAdmin}/account`:
      temp[0] = true
      break
    case `${systemConfig.prefixAdmin}/login-log`:
      temp[1] = true
      break
    case `${systemConfig.prefixAdmin}/category`:
      temp[2] = true
      break
    case `${systemConfig.prefixAdmin}/product`:
      temp[3] = true
      break
    case `${systemConfig.prefixAdmin}/user`:
      temp[4] = true
      break
  ...
  const handleClickMenu = (tab) => {
    switch (tab) {
    case 'account':
      navigate(`${systemConfig.prefixAdmin}/account`)
      break
    case 'login-log':
      navigate(`${systemConfig.prefixAdmin}/login-log`)
      break
    case 'category':
      navigate(`${systemConfig.prefixAdmin}/category`)
      break
    case 'product':
      navigate(`${systemConfig.prefixAdmin}/product`)
      break
    case 'user':
      navigate(`${systemConfig.prefixAdmin}/user`)
      break
    default:
      break
    }
